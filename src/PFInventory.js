...
import SWUtils from '../stubs/SWUtils';
import PFUtils from './PFUtils';
import PFMenus from './PFMenus';
import PFLog from './PFLog';
import PFConst from './PFConst';
import PFDefense from './PFDefense';
import PFAttackOptions from './PFAttackOptions';
import PFAttackGrid from './PFAttackGrid';
import PFMigrate from './PFMigrate';

export const wornEquipmentRowsOld = ["Belt", "Body", "Chest", "Eyes", "Feet", "Hands", "Head", "Headband", "Neck", "Ring1", "Ring2", "Shoulders", "Wrist"];
export const wornEquipmentRowsNew = ["Armor", "Belt", "Body", "Chest", "Eyes", "Feet", "Hands", "Head", "Headband", "Neck", "Ring1", "Ring2", "Shield", "Shoulders", "Wrist"];
const wornEquipmentRowsPlusCarried=["Carried","NotCarried"].concat(wornEquipmentRowsNew);
const locationMap = {'Carried':0,'NotCarried':1,'Armor':2,'Belt':3,'Body':4,'Chest':5,'Eyes':6,'Feet':7,'Hands':8,
  'Head':9,'Headband':10,'Neck':11,'Ring1':12,'Ring2':13,'Shield':14,'Shoulders':15,'Wrist':16};
const equipMap = {'noEquipType':0,'Weapon':1,'Armor':2,'Ammo':3,'Consumables':4,'OtherMagic':5,'Gear':6,'Other':7};
const groupMapForMenu = {0:'',1:'weapons',2:'armor-shield',3:'ammunition',4:'consumables',5:'other-magic-items',6:'gear-tool',7:'other-items'};
export const wornEquipmentColumns = ["charges", "weight", "hp", "hp_max", "value"];
export const commonLinkedAttributes = ["attack-type", "range", "masterwork", "crit-target", "crit-multiplier", "damage-dice-num", "damage-die", "damage",
...
*/
export function updateRepeatingItems (callback, silently) {
  var done = _.once(function () {
    if (typeof callback === "function") {
      callback();
    }
...
*@param {bool} silently if true send PFConst.silentParams to setAttrs
*/
export function updateCarriedCurrency (callback, silently) {
  var done = function () {
    if (typeof callback === "function") {
      callback();
    }
...
*@param {bool} silently if true send PFConst.silentParams to setAttrs
*/
export function updateCarriedTotal (callback, silently) {
  var done = function () {
    if (typeof callback === "function") {
      callback();
    }
...
*@param {bool} silently if true send PFConst.silentParams to setAttrs
*/
export function migrateWornEquipment (callback) {
...
*/
export function updateEquipmentLocation (id, callback, silently, eventInfo) {
  var done = _.once(function () {
...
*/
export function updateWornArmorAndShield (location, sourceAttribute, callback) {
  var done = _.once(function () {
